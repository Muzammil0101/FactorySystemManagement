...
// //                  <button onClick={() => setEntryForm({...entryForm, type: "credit"})} className={`py-2 rounded-lg text-sm font-bold transition-all ${entryForm.type === "credit" ? "bg-white shadow-sm text-emerald-700" : "text-slate-500 hover:bg-white/50"}`}>Credit (Received)</button>
// //               </div>
// //               <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Amount (PKR)</label><div className="relative"><Banknote className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="number" value={entryForm.amount} onChange={e => setEntryForm({...entryForm, amount: e.target.value})} className={`w-full bg-slate-50 border rounded-xl pl-11 pr-4 py-3 text-lg font-bold text-slate-800 focus:ring-2 outline-none transition-all ${entryForm.type === 'debit' ? 'focus:ring-rose-500/20 focus:border-rose-500 border-rose-100' : 'focus:ring-emerald-500/20 focus:border-emerald-500 border-emerald-100'}`} placeholder="0.00" /></div></div>
// //               <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Date</label><div className="relative"><Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="date" value={entryForm.date} onChange={e => setEntryForm({...entryForm, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm text-slate-700 focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500 outline-none transition-all font-medium" /></div></div>
// //               <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Description</label><input type="text" placeholder="e.g. Opening Balance" value={entryForm.description} onChange={e => setEntryForm({...entryForm, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500 outline-none transition-all font-medium" /></div>
// //               <button onClick={handleAddEntry} className={`w-full text-white py-3.5 rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-2 mt-2 ${entryForm.type === 'debit' ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-200' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200'}`}><CheckCircle size={18} /> {entryForm.type === 'debit' ? 'Add Debit Entry' : 'Add Credit Entry'}</button>
// //             </div>
...
// //             </div>
// //             <div className="p-6 space-y-5">
// //               <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Amount (PKR)</label><div className="relative"><Banknote className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="number" value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-lg font-bold text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" placeholder="0.00" /></div></div>
// //               <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Date</label><div className="relative"><Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="date" value={paymentForm.date} onChange={e => setPaymentForm({...paymentForm, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium" /></div></div>
// //               <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">Note / Description</label><input type="text" placeholder="e.g. Cash Received" value={paymentForm.description} onChange={e => setPaymentForm({...paymentForm, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium" /></div>
// //               <button onClick={handleReceivePayment} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-blue-200 transition-all flex items-center justify-center gap-2 mt-2"><CheckCircle size={18} /> Confirm Receipt</button>
// //             </div>
...
//   const [searchTerm, setSearchTerm] = useState("");
//   const [selectedCustomer, setSelectedCustomer] = useState(null);
//   const [matchingSupplier, setMatchingSupplier] = useState(null); 
//   const [showLedger, setShowLedger] = useState(false);
//   const [showPaymentModal, setShowPaymentModal] = useState(false);
//   const [showContraModal, setShowContraModal] = useState(false); 
  
//   // Sorting & View State
//   const [sortOption, setSortOption] = useState("name"); // name, high-balance, low-balance
...
//         return calculateCustomerBalance(b) - calculateCustomerBalance(a); // High Receivable
//       } else if (sortOption === "low-balance") {
//         return calculateCustomerBalance(a) - calculateCustomerBalance(b); // High Advance (Negative)
//       }
//       return 0;
//     });
...
//   // Use visibleCount for "See More" instead of page-based pagination
//   const currentCustomers = processedCustomers.slice(0, visibleCount);
...
//   // --- LEDGER DATE FILTERING & OPENING BALANCE LOGIC ---
//   const getFilteredLedgerWithOpening = () => {
//     if (!selectedCustomer?.ledger) return { filtered: [], openingBalance: 0 };

//     const sorted = getSortedLedger(selectedCustomer.ledger);
...
//         // Customer: Debit (Sale) increases balance, Credit (Payment) decreases
...
//     // Add Opening Balance Row if filter is active
//     if (ledgerStartDate) {
...
//     link.setAttribute("download", `Ledger_${selectedCustomer.name}_${new Date().toISOString().slice(0,10)}.csv`);
...
//         <h2 style="text-align:center; margin-bottom:5px;">${selectedCustomer.name} - Ledger</h2>
...
//                 <td style="padding:8px;border:1px solid #e2e8f0;text-align:right;">${Math.abs(ledgerOpeningBalance).toLocaleString()} ${ledgerOpeningBalance >= 0 ? 'Recv' : 'Adv'}</td>
//             </tr>
//          `;
//     }
...
//           <td style="padding:8px;border:1px solid #e2e8f0;text-align:right;color:#ef4444;">${entry.debit ? entry.debit.toLocaleString() : '-'}</td>
//           <td style="padding:8px;border:1px solid #e2e8f0;text-align:right;color:#10b981;">${entry.credit ? entry.credit.toLocaleString() : '-'}</td>
//           <td style="padding:8px;border:1px solid #e2e8f0;text-align:right;font-weight:bold;">${Math.abs(runningBalance).toLocaleString()} ${runningBalance >= 0 ? 'Recv' : 'Adv'}</td>
//         </tr>
//       `;
//     });
...
//         <tfoot style="background:#f8fafc;font-weight:bold;">
//           <tr>
//             <td colspan="4" style="padding:10px;text-align:right;">View Totals</td>
//             <td style="padding:10px;text-align:right;color:#ef4444;">${totalDebit.toLocaleString()}</td>
//             <td style="padding:10px;text-align:right;color:#10b981;">${totalCredit.toLocaleString()}</td>
//             <td></td>
//           </tr>
...
//     const popup = window.open('', '_blank', 'width=900,height=700');
//     popup.document.write(`<html><head><title>${selectedCustomer.name}</title></head><body style="padding:40px;font-family:sans-serif;">${ledgerHTML}</body></html>`);
//     popup.document.close();
//   };

//   // Variable to track visual running balance in the modal map loop
...
//                 className="w-full pl-10 pr-8 py-3.5 bg-white border border-slate-200 rounded-2xl shadow-sm text-sm text-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none appearance-none cursor-pointer font-medium"
//              >
//                  <option value="name">Sort by Name</option>
//                  <option value="high-balance">Highest Receivable</option>
...
//                             <td className="p-4 text-right font-bold text-slate-700">
//                                 {Math.abs(ledgerOpeningBalance).toLocaleString()} <span className="text-[10px] text-slate-400 font-normal">{ledgerOpeningBalance >= 0 ? 'Recv' : 'Adv'}</span>
//                             </td>
//                             <td></td>
//                         </tr>
//                     )}
...
//                         // For customers: Debit (Sale) increases balance, Credit (Payment) decreases balance
//                         renderRunningBalance += (entry.debit || 0) - (entry.credit || 0);
//                         return (
//                           <tr key={entry.id || idx} className="group hover:bg-slate-50/50 transition-colors">
//                             {editingEntry === entry.id ? (
...
//                       <td className="p-4 text-right text-rose-700">Rs. {ledgerViewData.reduce((sum, e) => sum + (e.debit || 0), 0).toLocaleString()}</td>
//                       <td className="p-4 text-right text-emerald-700">Rs. {ledgerViewData.reduce((sum, e) => sum + (e.credit || 0), 0).toLocaleString()}</td>
//                       <td colSpan="2"></td>
//                     </tr>
//                     <tr className={renderRunningBalance >= 0 ? "bg-blue-50" : "bg-emerald-50"}>
//                       <td colSpan="6" className="p-4 text-right text-slate-700 uppercase text-xs tracking-wider">Net Balance</td>
//                       <td colSpan="2" className={`p-4 text-right text-lg font-extrabold ${renderRunningBalance >= 0 ? 'text-blue-700' : 'text-emerald-700'}`}>
//                         Rs. {Math.abs(renderRunningBalance).toLocaleString()}
//                         <span className="text-xs font-medium ml-2 opacity-80">{renderRunningBalance >= 0 ? '(RECEIVABLE)' : '(ADVANCE)'}</span>
//                       </td>
//                     </tr>
//                   </tfoot>
//                 </table>
...
//                         className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-2.5 text-sm text-slate-600 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200"
//                       />
//                   </div>
//               </div>
...
export default function CustomersPage() {
  ...
  // --- UPDATED CONTRA SETTLEMENT LOGIC ---
  const handleContraSettlement = async () => {
    if (!contraForm.amount || parseFloat(contraForm.amount) <= 0) {
      showNotification("Please enter a valid amount", "error");
      return;
    ...
    setLoading(true);
    try {
      // Use the new single transaction route
      const response = await fetch(`${API_URL}/contra/settle`, {
        ...
        body: JSON.stringify({
          ...
          amount: contraForm.amount,
          date: contraForm.date,
          description: contraForm.description, // Added
          weight: contraForm.weight,           // Added
          rate: contraForm.rate                // Added
        })
      });
  ...
  const processedCustomers = useMemo(() => {
    ...
    result.sort((a, b) => {
      if (sortOption === "name") {
        ...
        return calculateCustomerBalance(b) - calculateCustomerBalance(a); 
      } else if (sortOption === "low-balance") {
        return calculateCustomerBalance(a) - calculateCustomerBalance(b); 
      }
      return 0;
    });
                ...
                {matchingSupplier && (
                  <>
                    ...
                    <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700 text-white flex flex-col justify-between">
                      ...
                      <button 
                        onClick={() => { 
                          setContraForm({ amount: "", date: new Date().toISOString().split('T')[0], description: "", weight: "", rate: "" }); 
              ...
              <div>
                  ...
                  <div className="relative">
                      ...
                      <input 
                        type="file"
                        onChange={(e) => setPaymentForm({...paymentForm, attachment: e.target.files[0]})}
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-2.5 text-sm text-slate-600 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium"
                      />
                  </div>
              </div>
