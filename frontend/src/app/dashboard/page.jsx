...
//   // 1. Feature 3: Today's Overview (Daily Pulse)
//   const today = new Date().toISOString().split('T')[0];
  
//   const salesToday = stockOut.filter(item => item.date === today);
//   const salesTodayAmount = salesToday.reduce((sum, item) => sum + (parseFloat(item.weight || 0) * parseFloat(item.rate || 0)), 0);
//   const salesTodayCount = salesToday.length;

//   const purchasesToday = stockIn.filter(item => item.date === today);
//   const purchasesTodayAmount = purchasesToday.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0);
//   const purchasesTodayCount = purchasesToday.length;

//   // 2. Feature 5: Top Moving Items (Current Month)
//   const currentMonthIdx = new Date().getMonth();
//   const currentYear = new Date().getFullYear();
  
//   const salesThisMonth = stockOut.filter(item => {
...
//     itemSalesMap[name] += parseFloat(item.weight || 0);
//   });
...
//   // 3. Feature 7: Month-End Closure Reminder
//   const dayOfMonth = new Date().getDate();
//   const daysInMonth = new Date(currentYear, currentMonthIdx + 1, 0).getDate();
//   const daysLeft = daysInMonth - dayOfMonth;
//   const showClosureAlert = dayOfMonth > 25; // Show if past the 25th
...
//       {/* Feature 7: Month-End Closure Reminder */}
//       {showClosureAlert && (
//         <div className="relative z-20 mb-6 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4 flex items-center justify-between shadow-sm animate-pulse-slow">
//           <div className="flex items-center gap-3">
//             <div className="p-2 bg-amber-100 rounded-lg text-amber-700">
//               <AlertTriangle size={20} />
//             </div>
//             <div>
//               <h3 className="text-sm font-bold text-amber-900">Month-End Closure Approaching</h3>
//               <p className="text-xs text-amber-700">
//                 {daysLeft <= 0 ? "Today is the last day!" : `${daysLeft} days left in this month.`} Prepare invoices and stock checks.
//               </p>
//             </div>
//           </div>
//           <a href="/stock-transfer" className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-lg transition-colors">
//             Prepare Closure
//           </a>
//         </div>
//       )}
...
//       {/* Feature 3: Today's Pulse (Daily Overview) */}
//       <div className="relative z-10 mb-8">
//         <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4 flex items-center gap-2">
//           <Zap size={16} className="text-yellow-500" /> Today's Pulse
//         </h3>
//         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//           <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-md flex items-center justify-between group hover:border-emerald-200 transition-all">
//             <div className="flex items-center gap-3">
//               <div className="p-2 bg-emerald-50 text-emerald-600 rounded-xl group-hover:bg-emerald-100 transition-colors">
//                 <ArrowUpRight size={20} />
//               </div>
//               <div>
//                 <p className="text-xs text-slate-500 font-bold uppercase">Sales Today</p>
//                 <div className="flex items-baseline gap-2">
//                   <span className="text-xl font-extrabold text-emerald-700">₨{salesTodayAmount.toLocaleString()}</span>
//                   <span className="text-xs text-emerald-500 font-medium">({salesTodayCount} txns)</span>
//                 </div>
//               </div>
//             </div>
//           </div>

//           <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-md flex items-center justify-between group hover:border-rose-200 transition-all">
//             <div className="flex items-center gap-3">
//               <div className="p-2 bg-rose-50 text-rose-600 rounded-xl group-hover:bg-rose-100 transition-colors">
//                 <TrendingDown size={20} />
//               </div>
//               <div>
//                 <p className="text-xs text-slate-500 font-bold uppercase">Purchases Today</p>
//                 <div className="flex items-baseline gap-2">
//                   <span className="text-xl font-extrabold text-rose-700">₨{purchasesTodayAmount.toLocaleString()}</span>
//                   <span className="text-xs text-rose-500 font-medium">({purchasesTodayCount} txns)</span>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//                 <Area type="monotone" dataKey="stockIn" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorIn)" name="Stock In (kg)" />
//                 <Area type="monotone" dataKey="stockOut" stroke="#f43f5e" strokeWidth={3} fillOpacity={1} fill="url(#colorOut)" name="Stock Out (kg)" />
//               </AreaChart>
//             </ResponsiveContainer>
//           </div>
//         </div>
...
//                       ))}
//                     </Pie>
//                     <Tooltip contentStyle={{ backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} formatter={(value) => `${Number(value).toFixed(2)} kg`} />
//                   </PieChart>
//                 </ResponsiveContainer>
//                 <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
//                   <span className="text-3xl font-bold text-slate-800">{categoryData.length}</span>
...
//         {/* Feature 5: Top Selling Products */}
//         <div className="bg-white border border-slate-100 rounded-3xl shadow-xl shadow-slate-200/50 p-6">
//           <div className="flex items-center justify-between mb-6">
//             <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
//               <Trophy size={16} className="text-yellow-500" /> Best Selling Products
//             </h3>
//             <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg">This Month</span>
//           </div>
//           <div className="space-y-3">
//             {topSellers.map((item, idx) => (
//               <div key={idx} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 hover:border-yellow-200 transition-colors group">
//                 <div className="flex items-center gap-3">
//                   <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs ${idx === 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-200 text-slate-600'}`}>
//                     #{idx + 1}
//                   </div>
//                   <span className="font-bold text-slate-700">{item.name}</span>
//                 </div>
//                 <div className="text-right">
//                   <span className="text-xs font-bold text-slate-500 block">Sold</span>
//                   <span className="text-sm font-bold text-yellow-600">{item.weight.toFixed(0)} kg</span>
//                 </div>
//               </div>
//             ))}
//             {topSellers.length === 0 && <p className="text-center text-slate-400 py-4 italic text-sm">No sales yet this month</p>}
//           </div>
//         </div>

//         {/* Quick Stats / Overview - Light Theme */}
//         <div className="bg-white border border-slate-100 rounded-3xl shadow-xl shadow-slate-200/50 p-6 flex flex-col">
//           <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
//             <Activity size={20} className="text-indigo-600" /> Performance Overview
//           </h3>
          
//           <div className="space-y-4">
//             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
//               <div className="flex justify-between items-center mb-1">
//                 <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Inflow</span>
//                 <TrendingUp size={14} className="text-emerald-500" />
//               </div>
//               <p className="text-2xl font-extrabold text-emerald-600">{summary.totalIn.toFixed(2)} <span className="text-sm text-slate-400 font-medium">kg</span></p>
//             </div>

//             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
//               <div className="flex justify-between items-center mb-1">
//                 <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Outflow</span>
//                 <TrendingDown size={14} className="text-rose-500" />
//               </div>
//               <p className="text-2xl font-extrabold text-rose-600">{summary.totalOut.toFixed(2)} <span className="text-sm text-slate-400 font-medium">kg</span></p>
//             </div>

//             <div className="pt-4">
//               <div className="flex justify-between text-sm font-bold text-slate-600 mb-2">
//                 <span>Stock Utilization</span>
//                 <span className="text-indigo-600">{((summary.totalOut / (summary.totalIn || 1)) * 100).toFixed(1)}%</span>
//               </div>
//               <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden border border-slate-100">
//                 <div 
//                   className="bg-gradient-to-r from-indigo-500 to-purple-600 h-full rounded-full shadow-sm" 
//                   style={{ width: `${Math.min(((summary.totalOut / (summary.totalIn || 1)) * 100), 100)}%` }}
//                 ></div>
//               </div>
//             </div>
...
export default function DashboardPage() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      // --- FILTERING LOGIC ---
      // We filter out the Month End Transfers so they don't skew the charts or "Best Selling" lists
      const realPurchases = (purchasesData || []).filter(item => item.supplier !== 'Monthly Transfer');
      const realSales = (salesData || []).filter(item => item.customer !== 'Month End Transfer');

      setStockIn(realPurchases);
      setStockOut(realSales);
      
      setCustomers(customersData || []);
      setSuppliers(suppliersData || []);
      setCategories(categoriesData || []);
      
      // Note: We use stockData.currentStock from backend because it accurately calculates 
      // physical stock (Total In [including transfers] - Total Out [including transfers])
      setSummary({
        totalIn: stockData.totalStockIn || 0,
        totalOut: stockData.totalStockOut || 0,
        currentStock: stockData.currentStock || 0
        ...
        <div className="bg-white border border-slate-100 rounded-3xl shadow-xl shadow-slate-200/50 p-6 flex flex-col">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <p className="text-2xl font-extrabold text-emerald-600">{stockIn.reduce((sum, item) => sum + parseFloat(item.weight || 0), 0).toFixed(2)} <span className="text-sm text-slate-400 font-medium">kg</span></p>
            </div>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <p className="text-2xl font-extrabold text-rose-600">{stockOut.reduce((sum, item) => sum + parseFloat(item.weight || 0), 0).toFixed(2)} <span className="text-sm text-slate-400 font-medium">kg</span></p>
            </div>
            ...
            <div className="pt-4">
              <div className="flex justify-between text-sm font-bold text-slate-600 mb-2">
                ...
                <span className="text-indigo-600">
                  {stockIn.length > 0 
                    ? ((stockOut.reduce((sum, item) => sum + parseFloat(item.weight || 0), 0) / stockIn.reduce((sum, item) => sum + parseFloat(item.weight || 0), 0)) * 100).toFixed(1) 
                    : 0}%
                </span>
              </div>
              <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden border border-slate-100">
                <div 
                  className="bg-gradient-to-r from-indigo-500 to-purple-600 h-full rounded-full shadow-sm" 
                  style={{ width: `${Math.min(((stockOut.reduce((sum, item) => sum + parseFloat(item.weight || 0), 0) / (stockIn.reduce((sum, item) => sum + parseFloat(item.weight || 0), 0) || 1)) * 100), 100)}%` }}
                ></div>
              </div>
            </div>
          </div>
