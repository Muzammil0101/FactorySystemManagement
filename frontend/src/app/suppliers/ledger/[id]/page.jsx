...
export default function SupplierLedgerPage({ params }) {
  ...
  const handleDelete = (sr) => {
    if (confirm("Are you sure you want to delete this entry?")) {
      setEntries(entries.filter((e) => e.sr !== sr));
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold text-slate-800">Supplier Ledger</h1>
              </div>
              <p className="text-slate-500 mt-1">
                {supplierName} • Supplier ID: #{id}
              </p>
            </div>
            <button
        ...
        {showForm && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Debit (Rs.)</label>
                <input
                  placeholder="Amount"
                  type="number"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
                ...
                {dataWithBalance.length === 0 ? (
                  <tr>
                    <td colSpan="9" className="px-6 py-12 text-center">
                      <p className="text-slate-500 font-medium">No entries added yet</p>
                      <p className="text-slate-400 text-sm mt-1">
                        Click "New Entry" to add your first transaction
                      </p>
                    </td>
                  </tr>
                ) : (
                  dataWithBalance.map((e) => (
                    <tr key={e.sr} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-600 text-right">
                        {e.rate > 0 ? `Rs. ${parseFloat(e.rate).toLocaleString()}` : '-'}
                      </td>
                      <td className="px-6 py-4 text-sm text-red-600 font-semibold text-right">
                        {e.debit > 0 ? `Rs. ${parseFloat(e.debit).toLocaleString()}` : '-'}
                      </td>
                      <td className="px-6 py-4 text-sm text-green-600 font-semibold text-right">
                        {e.credit > 0 ? `Rs. ${parseFloat(e.credit).toLocaleString()}` : '-'}
                      </td>
                      <td className="px-6 py-4 text-sm font-bold text-right">
                        <span className={parseFloat(e.balance) >= 0 ? 'text-red-600' : 'text-green-600'}>
                          Rs. {Math.abs(parseFloat(e.balance)).toLocaleString()}
                          <span className="text-xs ml-1">
                            {parseFloat(e.balance) >= 0 ? 'Dr' : 'Cr'}
                          </span>
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => handleDelete(e.sr)}
