...
export default function CategoryPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const categoriesWithStats = safeCatData.map(cat => {
        ...
        // --- NEW: FETCH LATEST DETAILS FROM STOCK PAGE ---
        // Combine all transactions
        const allTransactions = [...relevantInAll, ...relevantOutAll];
        // Sort by ID descending (assuming higher ID is newer)
        allTransactions.sort((a, b) => b.id - a.id);
        // Find the first transaction that has details
        const latestTrans = allTransactions.find(t => t.details && t.details.trim().length > 0);
        const stockDetails = latestTrans ? latestTrans.details : null;
        ...
        return {
          ...
          stockDetails: stockDetails // Store the fetched details
        };
      });
  ...
  const filtered = categories.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase()) ||
    (item.stockDetails && item.stockDetails.toLowerCase().includes(search.toLowerCase())) ||
    (item.description && item.description.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  const handleAdd = async () => {
    ...
    // Note: We keep this required here for manual entry, but the display will prefer stock details
    if (!form.description.trim()) {
      showNotification("Description / Details is required!", "error");
      return;
    }
  ...
  const exportCSV = () => {
    ...
    const csv = [
      ["Category Name", "Latest Details (Stock)", "Stock In (kg)", "Stock Out (kg)", "Current Stock (kg)"],
      ...categories.map((c) => [
        c.name, 
        c.stockDetails || c.description || "", // Export the stock details if available
        c.stockIn?.toFixed(2) || "0.00",
        c.stockOut?.toFixed(2) || "0.00",
        c.currentStock?.toFixed(2) || "0.00"
      ]),
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="relative group">
                    <AlignLeft className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={18} />
                    <input
                      type="text"
                      placeholder="Category details, notes..."
                      value={form.description}
                      onChange={(e) => setForm({ ...form, description: e.target.value })}
                      disabled={loading}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all placeholder:text-slate-400 font-medium"
              ...
              <div key={cat.id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:border-blue-200 transition-all duration-300 group">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-800 text-lg leading-tight">{cat.name}</h3>
                      {/* PREFER STOCK DETAILS OVER CATEGORY DESCRIPTION */}
                      <p className="text-xs text-slate-500 mt-0.5">
                        {cat.stockDetails || cat.description || "No recent details"}
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
