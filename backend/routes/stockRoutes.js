...
// Add Stock In (or Expense)
router.post("/stock-in", (req, res) => {
  const { date, description, details, weight, rate, amount, supplier, is_expense } = req.body;

  try {
    if (!date || !description || !amount) {
      return res.status(400).json({ error: "Date, Item and amount are required" });
    }

    // NEW: Check for Details
    if (!details) {
        return res.status(400).json({ error: "Description/Notes is required!" });
    }
...
// Add Stock Out (Sale or Return)
router.post("/stock-out", (req, res) => {
  const { date, description, details, weight, rate, purchase_rate, amount, customer, is_return } = req.body;

  try {
    if (!date || !description || !customer) {
      return res.status(400).json({ error: "Date, Item and customer are required" });
    }
    // NEW: Check for Details
    if (!details) {
        return res.status(400).json({ error: "Description/Notes is required!" });
