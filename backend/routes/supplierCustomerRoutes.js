...
router.post('/contra/settle', (req, res) => {
  const { customerId, supplierId, amount, date, weight, rate, description } = req.body;
  ...
  try {
    ...
    const finalDesc = description && description.trim() !== "" 
      ? description 
      : `Contra Settlement: Offset between ${customer.name} & ${supplier.name}`;

    // 3. START TRANSACTION (Ensures both happen or neither happens)
    const contraTransaction = db.transaction(() => {
