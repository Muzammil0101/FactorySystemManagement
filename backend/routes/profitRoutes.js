...
router.get('/profit-loss/balance-sheet', (req, res) => {
  try {
    ...
    const stockIn = db.prepare(stockInQuery).get(...dateFilter.params);
    const stockOut = db.prepare(stockOutQuery).get(...dateFilter.params);
    ...
    const customerList = customers.map(c => {
       ...
       const ledger = db.prepare(ledgerQuery).get(...params);
       const debit = ledger.d || 0;
       const credit = ledger.c || 0;
       return { 
         name: c.name, 
...
router.get('/profit-loss/by-category', (req, res) => {
  try {
    ...
    const results = categories.map(cat => {
      ...
      const inData = db.prepare(inQuery).get(cat.description, ...dateFilter.params);
      const outData = db.prepare(outQuery).get(cat.description, ...dateFilter.params);
...
router.get('/profit-loss/by-supplier', (req, res) => {
  try {
    ...
    const results = suppliers.map(sup => {
      ...
      `;
      const costData = db.prepare(query).get(sup.supplier, ...dateFilter.params);
...
router.get('/profit-loss/by-customer', (req, res) => {
  try {
    ...
    const results = customers.map(cust => {
      ...
      `;
      const revenueData = db.prepare(query).get(cust.customer, ...dateFilter.params);
