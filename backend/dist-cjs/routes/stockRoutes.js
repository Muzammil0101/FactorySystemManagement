...
router.get("/debug/stock-data", (req, res) => {
  try {
    const stockInItems = _database.default.prepare("SELECT id, description, weight FROM stock_in").all();
    const stockOutItems = _database.default.prepare("SELECT id, description, weight FROM stock_out").all();
    const categories = _database.default.prepare("SELECT id, name FROM categories").all();
    res.json({
      categories,
      stockInItems,
      stockOutItems,
...
router.get("/categories", (req, res) => {
  try {
    const categories = _database.default.prepare("SELECT * FROM categories ORDER BY name ASC").all();
    const categoriesWithStock = categories.map(cat => {
      const stockInData = _database.default.prepare(`
          SELECT COALESCE(SUM(CAST(weight AS REAL)), 0) AS total
          FROM stock_in 
          WHERE LOWER(TRIM(description)) = LOWER(TRIM(?))
          AND supplier != 'Monthly Transfer'
      `).get(cat.name);
      const stockOutData = _database.default.prepare(`
          SELECT COALESCE(SUM(CAST(weight AS REAL)), 0) AS total
          FROM stock_out 
          WHERE LOWER(TRIM(description)) = LOWER(TRIM(?))
          AND customer != 'Month End Transfer'
...
router.post("/categories", (req, res) => {
  ...
  try {
    ...
    const existing = _database.default.prepare("SELECT * FROM categories WHERE name = ?").get(name);
    if (existing) {
      return res.status(400).json({
    ...
    const result = _database.default.prepare(`
        ...
        VALUES (?, ?)
    `).run(name, description || "");
    const newCategory = _database.default.prepare("SELECT * FROM categories WHERE id = ?").get(result.lastInsertRowid);
    res.json({
      success: true,
      message: `Category "${name}" added successfully`,
      data: {
...
router.put("/categories/:id", (req, res) => {
  ...
  try {
    ...
    const existing = _database.default.prepare("SELECT * FROM categories WHERE id = ?").get(id);
    if (!existing) {
      return res.status(404).json({
        error: "Category not found"
      });
    }
    const duplicate = _database.default.prepare("SELECT * FROM categories WHERE name = ? AND id != ?").get(name, id);
    if (duplicate) {
      return res.status(400).json({
    ...
    _database.default.prepare(`
      ...
      WHERE id = ?
    `).run(name, description || "", id);
    const updated = _database.default.prepare("SELECT * FROM categories WHERE id = ?").get(id);
...
router.delete("/categories/:id", (req, res) => {
  ...
  try {
    const category = _database.default.prepare("SELECT * FROM categories WHERE id = ?").get(id);
    if (!category) {
      return res.status(404).json({
        error: "Category not found"
      });
    }
    const inStockIn = _database.default.prepare("SELECT COUNT(*) as count FROM stock_in WHERE description = ?").get(category.name);
    const inStockOut = _database.default.prepare("SELECT COUNT(*) as count FROM stock_out WHERE description = ?").get(category.name);
    if (inStockIn.count > 0 || inStockOut.count > 0) {
      return res.status(400).json({
        error: `Cannot delete "${category.name}" because it is in use`
      });
    }
    _database.default.prepare("DELETE FROM categories WHERE id = ?").run(id);
    res.json({
      success: true,
      message: `Category "${category.name}" deleted successfully`
    });
...
router.post("/transfer", (req, res) => {
  ...
  try {
    const outDate = new Date().toISOString().split('T')[0];
    _database.default.prepare(`
      INSERT INTO stock_out (date, description, weight, rate, purchase_rate, amount, customer, details)
      VALUES (?, ?, ?, 0, 0, 0, 'Month End Transfer', 'System Transfer')
    `).run(outDate, description, currentWeight);
    _database.default.prepare(`
      INSERT INTO stock_in (date, description, weight, rate, amount, supplier, details)
      VALUES (?, ?, ?, ?, ?, ?, 'System Transfer')
    `).run(date, description, newWeight, rate, amount, 'Monthly Transfer');
    _database.default.prepare(`
      INSERT OR IGNORE INTO categories (name, description)
      VALUES (?, '')
...
router.get("/stock-in", (req, res) => {
  try {
    const stockIn = _database.default.prepare("SELECT * FROM stock_in ORDER BY date DESC").all();
    res.json(stockIn);
  } catch (error) {
    res.status(500).json({
      error: error.message
...
router.post("/stock-in", (req, res) => {
  ...
  try {
    ...
    if (!details) {
      return res.status(400).json({
        error: "Description/Notes is required!"
      });
    }
    ...
    const finalSupplier = supplier || (is_expense ? 'General Expense' : 'Unknown');
    const result = _database.default.prepare(`
      INSERT INTO stock_in (date, description, details, weight, rate, amount, supplier)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(date, description, details, finalWeight, rate || 0, amount, finalSupplier);
    ...
    // Auto-create supplier (even if expense, we might want to track payee)
    const existingSupplier = _database.default.prepare("SELECT * FROM suppliers WHERE name = ?").get(finalSupplier);
    if (!existingSupplier) {
      _database.default.prepare(`
        INSERT INTO suppliers (name, phone, city)
        VALUES (?, '', '')
    ...
    _database.default.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 0, ?)
    `).run(finalSupplier, date, description, finalWeight, rate || 0, amount);
    const newRecord = _database.default.prepare("SELECT * FROM stock_in WHERE id = ?").get(result.lastInsertRowid);
    res.json({
      success: true,
      message: is_expense ? `Expense recorded: ${description}` : `Added ${weight} kg of ${description}`,
      data: newRecord
...
router.put("/stock-in/:id", (req, res) => {
  ...
  try {
    const existing = _database.default.prepare("SELECT * FROM stock_in WHERE id = ?").get(id);
    if (!existing) {
      return res.status(404).json({
    ...
    _database.default.prepare(`
      UPDATE stock_in
      SET date = ?, description = ?, details = ?, weight = ?, rate = ?, amount = ?, supplier = ?
      WHERE id = ?
    `).run(date, description, details, finalWeight, rate, amount, finalSupplier, id);
    if (finalWeight > 0) {
      _database.default.prepare(`INSERT OR IGNORE INTO categories (name, description) VALUES (?, '')`).run(description);
    }
    const existingSupplier = _database.default.prepare("SELECT * FROM suppliers WHERE name = ?").get(finalSupplier);
    if (!existingSupplier) {
      _database.default.prepare(`INSERT INTO suppliers (name, phone, city) VALUES (?, '', '')`).run(finalSupplier);
    }
    ...
    _database.default.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 0, ?)
    `).run(finalSupplier, date, description, finalWeight, rate || 0, amount);
    const updated = _database.default.prepare("SELECT * FROM stock_in WHERE id = ?").get(id);
    res.json({
      success: true,
      message: "Record updated successfully",
      data: updated
...
router.delete("/stock-in/:id", (req, res) => {
  try {
    ...
    const record = _database.default.prepare("SELECT * FROM stock_in WHERE id = ?").get(id);
    if (!record) {
      return res.status(404).json({
        error: "Record not found"
      });
    }
    _database.default.prepare("DELETE FROM stock_in WHERE id = ?").run(id);
    _database.default.prepare(`
      DELETE FROM supplier_ledger
      WHERE supplier_name = ? AND date = ? AND description = ? AND credit = ?
      ORDER BY id DESC LIMIT 1
...
router.get("/stock-out", (req, res) => {
  try {
    const stockOut = _database.default.prepare("SELECT * FROM stock_out ORDER BY date DESC").all();
    res.json(stockOut);
  } catch (error) {
    res.status(500).json({
      error: error.message
...
router.post("/stock-out", (req, res) => {
  ...
  try {
    if (!date || !description || !customer) {
      return res.status(400).json({
        error: "Date, Item and customer are required"
      });
    }
    // NEW: Check for Details
    if (!details) {
    ...
    if (is_return) {
      ...
      // Normal Sale: Check Stock for THIS CATEGORY
      const catIn = _database.default.prepare("SELECT SUM(weight) as total FROM stock_in WHERE description = ?").get(description);
      const catOut = _database.default.prepare("SELECT SUM(weight) as total FROM stock_out WHERE description = ?").get(description);
      const currentStock = (catIn.total || 0) - (catOut.total || 0);
      if (finalWeight > currentStock) {
        return res.status(400).json({
          error: `Insufficient stock for ${description}! Available: ${currentStock.toFixed(2)} kg`
    ...
    // Insert
    const result = _database.default.prepare(`
      INSERT INTO stock_out (date, description, details, weight, rate, purchase_rate, amount, customer)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(date, description, details, finalWeight, rate, purchase_rate || 0, finalAmount, customer);
    ...
    // Auto-create customer
    const existingCustomer = _database.default.prepare("SELECT * FROM customers WHERE name = ?").get(customer);
    if (!existingCustomer) {
      _database.default.prepare(`
        INSERT INTO customers (name, phone, city)
        VALUES (?, '', '')
    ...
    const newRecord = _database.default.prepare("SELECT * FROM stock_out WHERE id = ?").get(result.lastInsertRowid);
    res.json({
      success: true,
      message: is_return ? `Return recorded for ${description}` : `Sold ${weight} kg of ${description}`,
      data: newRecord
...
router.put("/stock-out/:id", (req, res) => {
  ...
  try {
    // Get existing record to clean up ledger
    const existing = _database.default.prepare("SELECT * FROM stock_out WHERE id = ?").get(id);
    if (!existing) {
      return res.status(404).json({
        error: "Record not found"
      });
    ...
    if (is_return) {
      ...
      const catIn = _database.default.prepare("SELECT SUM(weight) as total FROM stock_in WHERE description = ?").get(description);
      // Sum of all OUTs for this category except current one
      const catOut = _database.default.prepare("SELECT SUM(weight) as total FROM stock_out WHERE description = ? AND id != ?").get(description, id);
      const currentStock = (catIn.total || 0) - (catOut.total || 0);
      if (finalWeight > currentStock) {
        return res.status(400).json({
          error: `Insufficient stock for ${description}! Available: ${currentStock.toFixed(2)} kg`
    ...
    _database.default.prepare(`
      UPDATE stock_out
      SET date = ?, description = ?, details = ?, weight = ?, rate = ?, purchase_rate = ?, amount = ?, customer = ?
      WHERE id = ?
    `).run(date, description, details, finalWeight, rate, purchase_rate || 0, finalAmount, customer, id);
    _database.default.prepare(`INSERT OR IGNORE INTO categories (name, description) VALUES (?, '')`).run(description);
    const existingCustomer = _database.default.prepare("SELECT * FROM customers WHERE name = ?").get(customer);
    if (!existingCustomer) {
      _database.default.prepare(`INSERT INTO customers (name, phone, city) VALUES (?, '', '')`).run(customer);
    }
    ...
    const updated = _database.default.prepare("SELECT * FROM stock_out WHERE id = ?").get(id);
    res.json({
      success: true,
      message: "Record updated successfully",
      data: updated
...
router.delete("/stock-out/:id", (req, res) => {
  try {
    ...
    const record = _database.default.prepare("SELECT * FROM stock_out WHERE id = ?").get(id);
    if (!record) {
      return res.status(404).json({
        error: "Record not found"
      });
    }
    _database.default.prepare("DELETE FROM stock_out WHERE id = ?").run(id);
...
router.get("/summary", (req, res) => {
  try {
    const totalIn = _database.default.prepare("SELECT SUM(weight) as total FROM stock_in").get();
    const totalOut = _database.default.prepare("SELECT SUM(weight) as total FROM stock_out").get();
    res.json({
      totalIn: totalIn.total || 0,
      totalOut: totalOut.total || 0,
      currentStock: (totalIn.total || 0) - (totalOut.total || 0)
...
router.get("/by-category", (req, res) => {
  try {
    const stockInByCategory = _database.default.prepare(`
        SELECT description, SUM(weight) AS total_in
        FROM stock_in
        GROUP BY description
    `).all();
    const stockOutByCategory = _database.default.prepare(`
        SELECT description, SUM(weight) AS total_out
        FROM stock_out
        GROUP BY description
    `).all();
...
router.get("/suppliers-with-ledger", (req, res) => {
  try {
    const suppliers = _database.default.prepare("SELECT * FROM suppliers").all();
    const result = suppliers.map(s => {
      const ledger = _database.default.prepare(`
        SELECT * FROM supplier_ledger
        WHERE supplier_name = ?
...
router.get("/customers-with-ledger", (req, res) => {
  try {
    const customers = _database.default.prepare("SELECT * FROM customers").all();
    const result = customers.map(c => {
      const ledger = _database.default.prepare(`
        SELECT * FROM customer_ledger
        WHERE customer_name = ?
...
router.get("/products", (req, res) => {
  try {
    const products = _database.default.prepare("SELECT * FROM products").all();
    res.json(products);
  } catch (error) {
    res.status(500).json({
      error: error.message
...
router.delete("/products/:id", (req, res) => {
  try {
    _database.default.prepare("DELETE FROM products WHERE id = ?").run(req.params.id);
    res.json({
      success: true
    });
  } catch (error) {
...
router.get("/transactions", (req, res) => {
  try {
    const transactions = _database.default.prepare("SELECT * FROM transactions ORDER BY date DESC").all();
    res.json(transactions);
  } catch (error) {
    res.status(500).json({
      error: error.message
